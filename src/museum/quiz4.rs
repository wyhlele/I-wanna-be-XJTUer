...
pub struct Quiz4Plugin;

impl Plugin for Quiz4Plugin{
    fn build(&self, app: &mut App){
        app.add_systems(PostStartup,spawn_once)
        .add_systems(OnExit(GameState::Reload),spawn_reload)
        .add_systems(Update,(do_accept).in_set(InGameSet::EntityUpdates));
    }
}
...
fn spawn_once(
    ...
    spawn_single_savepointer(&mut commands,&sv_image,&sv_atlas,0.,0.,BASEX,BASEY,7);
    ...
    let wp1 = spawn_single_warp(&mut commands,&wr_image,&wr_atlas,BASEX-224.,BASEY-256.,BASEX+800.,BASEY-608.);
    let wp2 = spawn_single_warp(&mut commands,&wr_image,&wr_atlas,BASEX,BASEY-256.,BASEX+800.,BASEY-608.);
    let wp3 = spawn_single_warp(&mut commands,&wr_image,&wr_atlas,BASEX+224.,BASEY-256.,BASEX+800.,BASEY-608.);
    

    commands.entity(wp1).insert(Accept);
    commands.entity(wp2).insert(Accept);
    commands.entity(wp3).insert(Accept);
}
